...
const FoundItemSubmission = () => {
  ...
  const handleSubmit = (e) => {
    ...
    foundItemSubmission(item)
      .then(() => {
        // Try to find matches with notification after successful submission
        // Use the enhanced matching with auto-notification
        findMatchesWithAutoNotify(item)
          .then((matchResponse) => {
            if (matchResponse.data && matchResponse.data.length > 0) {
              const highConfidenceMatches = matchResponse.data.filter(match => 
                (match.confidence || match.matchConfidence || 0) >= 70
              );
              if (highConfidenceMatches.length > 0) {
                setToastMessage(`Found Item submitted! ${highConfidenceMatches.length} owner(s) notified of potential matches.`);
                setToastType('success');
              } else {
                setToastMessage(`Found Item submitted! ${matchResponse.data.length} potential match(es) found but below notification threshold.`);
                setToastType('info');
              }
            } else {
              setToastMessage("Found Item submitted successfully! No matches found yet.");
              setToastType('info');
            }
            setShowToast(true);
          })
          .catch(() => {
            setToastMessage("Found Item submitted successfully!");
            setToastType('success');
            setShowToast(true);
          });
        getUserDetails()
